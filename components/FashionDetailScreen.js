...
const FashionDetailScreen = ({ route }) => {
  const { fashion } = route.params;  // Nhận dữ liệu từ màn hình trước
  ...
  const handleFavoritePress = () => {
    addToFavorites(fashion);  // Thêm sản phẩm vào yêu thích
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.title}>{fashion.name}</Text>
      ...
      {/* fashion Image */}
      <Image source={fashion.image} style={styles.fashionImage} />

      {/* fashion Price and Rating */}
      <View style={styles.priceRatingContainer}>
        <Text style={styles.price}>${fashion.price}</Text>
        <View style={styles.ratingContainer}>
          <Icon name="star" size={16} color="#FFD700" />
          <Text style={styles.ratingText}>{fashion.rating}</Text>
          <Text style={styles.reviewCount}>({fashion.reviews} đánh giá)</Text>
        </View>
      </View>

      {/* fashion Description */}
      <Text style={styles.sectionTitle}>Mô tả</Text>
      <Text style={styles.description}>{fashion.description}</Text>
